...
function print_result(t)
  io.write("{ ")
  for _,k in ipairs{'bad', 'bias', 'unknown', 'secs', 'pct', 'n'} do
    io.write(string.format("%q: %s, ", k, t[k] and math.floor(t[k]) or 'nil'))
  end
  io.write(" }\n")
end

function spelltest(tests, bias, verbose)
  local n, bad, unknown, start = 0, 0, 0, os.clock()
  if bias then for target in pairs(tests) do
      spell.model[target] = (spell.model[target] or 1) + bias
  end end
